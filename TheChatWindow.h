...
/*!
\file
\brief Заголовочный файл с основной формой программы

Данный файл содержит в себе описание основной формы чата и связанных с ней функций.
*/
...
#pragma endregion
	...
	*/
	...
	/*! \param s строка типа System::String
	\return str строка типа std::string
	*/
	...
	*/
	...
	/*! \brief Переносит список пользователей в чате во вкладку с чатом.
	\param froms Строка, содержащая список пользователей 
	\param cut Число, нужное для обрезки непечатных символов в конце строки, если это необходимо
	*/
	...
	*/
	...
	\param ChatIDStr Код чата, для которого нужно получить историю сообщений
	Формирует запрос и отправляет его на сервер. После того, как в другом потоке сообщения от сервера получены, обработаны
	...
	*/
	...
	/*! \param text Строка с сообщением, полученным от сервера
	*/
...
/*! \brief Переход во вкладку с чатом из списка
...
*/
	private: System::Void ChatListBox_DoubleClick(System::Object^ sender, System::EventArgs^ e) {
		...
		if (ChatListBox->SelectedIndex >= 0) {
			...
			tabControl1->SelectedIndex = 1;
			//! chatString - cтрока из пункта списка чатов для разбиения на код чата и список пользователей
			string chatString = SystemToStl(ChatListBox->SelectedItem->ToString());
			int position = 0; //! position - позиция в строке при разбиении её на части при помощи substr
